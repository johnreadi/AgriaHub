...
const Login: React.FC<LoginProps> = ({ onLoginSuccess }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            // Utiliser le service API central pour gérer les endpoints et le token
            const data = await apiService.login(username, password);

            if ((data && (data.success || data.access_token || data.token)) || apiService.isAuthenticated()) {
                ...
                setError((data && data.error) || 'Identifiants invalides');
            }
        } catch (error: any) {
