...
self.addEventListener('activate', event => {
  ...
  event.waitUntil(
    caches.keys()
      ...
      .then(() => {
        // Informer immédiatement les clients qu'une mise à jour est active
        return self.clients.matchAll().then(clients => {
          clients.forEach(client => {
            client.postMessage({
              type: 'SW_UPDATED',
              message: 'Service Worker activé et clients pris en charge'
            });
          });
        });
      })
...
self.addEventListener('fetch', event => {
  ...
  // Paramètres d'apparence: toujours frais et non cachés
