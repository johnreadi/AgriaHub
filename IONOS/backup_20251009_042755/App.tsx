  ...
  useEffect(() => {
    ...
    const fetchAppearance = async () => {
        try {
            ...
            const res = await fetch('/api/settings.php?type=appearance');
            if (!res.ok) {
                applyDefaultAppearance();
                return;
            }
            const data = await res.json();
            const settings = (data && (data.appearance || data)) as Partial<AppearanceSettings>;
      ...
      <div className="flex min-h-screen bg-transparent">
        ...
        <div className="flex-1 flex flex-col min-w-0 transition-all duration-300 ease-in-out">
          ...
          <main className="flex-1 w-full pb-20 md:pb-0">
              <ScrollReveal direction="up" className="bg-white p-4 sm:p-6 md:p-10 w-full relative z-0">
                   {renderContent()}
              </ScrollReveal>
          </main>
          <Footer />
